...
class MY_Lang extends CI_Lang {
    ...
    function __construct() {
        ...
        $country_list = $db->query($query)->result();

        $segment = $URI->segment(1);
        $segment2 = $URI->segment(2);
       
        foreach ($country_list as $country) {
            $this->languages[$country->short_code] = $country->short_code;
            if ($segment == $country->short_code) {
                $CFG->set_item('language', $country->short_code);
                $this->default_lang = $country->short_code;
                $this->default_lang_id = $country->id;
            }
            
            $this->languages['admin/' . $country->short_code] = $country->short_code;
            if ($segment == 'admin' && $segment2 == $country->short_code) {
        ...
        } else if(isset($this->languages[$segment . '/' . $segment2])){
            
        } else if (($this->is_special($segment)) or ( $segment == "admin")) {  // special URI -> no redirect

            if (strlen($segment) == 0) {
                header("Location: " . $CFG->base_url() . $this->default_lang . '/' . $URI->uri_string(), TRUE, 302);
            } else if ($segment == "admin") {
                if($segment2 == '') {
                    $seg = "admin/" . $this->default_lang . '/entry_door';
                    $adminurl = $CFG->base_url() . $seg;
                    header("Location: " . $adminurl, TRUE, 302);
                } else {
                    $seg = "admin/" . $this->default_lang . '/';
                    $adminurl = $CFG->base_url() . str_replace("admin/", $seg, $URI->uri_string());
                    header("Location: " . $adminurl, TRUE, 302);
                }
