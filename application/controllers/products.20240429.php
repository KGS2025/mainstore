...
class products extends MY_Controller
{
    ...
    {
//         echo "<pre>";print_r($this->input->post());var_dump($this->session->userdata('filter_option'));die;
        // Check user logged or not
        logged_user_validation();
    ...
    public function product_items($product_type_id = '')
    {
        ...
        $vehicle_category_id = $this->input->get('cid');
        $maker_id = $this->input->get('maid');
        $model_id = $this->input->get('moid');
...
//	echo '<pre>';print_r($this->session->userdata);exit;
	// this function executes when categories are not empty in the session
        if ($this->session->userdata('hide_category') == 0 || $this->session->userdata('search_cat') == 1) { // check category enabled or not based on the admin selection
	    if (!empty($vehicle_category_ids)) {
                ...
                foreach ($vehicle_category_ids as $key1 => $vehicle_category_id) {
                    ...
                    if (!in_array($vehicle_category_id, $previous_v_c_id)) {
                        ...
                        if ($cat_count > 0) {
                            ...
                            $cat_breadcrumbs .= ' - ' . '<a  class="bread_cat" href="' . base_url() . $this->lang->default_lang . '/products/vehicle_type/' . $vehicle_category_id . '">' . $categoryBreadcrumb[$vehicle_category_id] . '</a>';
                        } else {
                            // html string of category  breadcrumb
                            $cat_breadcrumbs .= '<a class="bread_cat" href="' . base_url() . $this->lang->default_lang . '/products/vehicle_type/' . $vehicle_category_id . '">' . $categoryBreadcrumb[$vehicle_category_id] . '</a>';
                        }
