...
/**
 * Adminuser
 * This Class Handle all functions related to Admin users like Add, Edit, List Delete etc.
 */
class Adminuser extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(array('comman_model', 'adminuser_model', 'adminrole_model'));
    ...
    /**
     * Method index
     * This Function list all users with pagination.
     * @return void
     */
    function index() {
        ...
        $access = validatePageAccess('adminuser');
        ...
        if ($key) {
            ...
            $config['base_url']    = base_url() . "admin/" . $this->lang->default_lang . "/adminuser/index/" . $key;
            $config['uri_segment'] = 5;
            $config['total_rows']  = $this->comman_model->record_search_count('admin_users', $key, array('email'));
            $offset                = ($this->uri->segment(5)) ? $this->uri->segment(5) : 0;
            $all_data              = $this->adminuser_model->search_admin_user_data($key, $config['per_page'], $offset, $this->lang->default_lang_id);
        } else {
            $config['base_url']    = base_url() . "admin/" . $this->lang->default_lang . "/adminuser/index/";
            $config['uri_segment'] = 4;
            $config['total_rows']  = $this->comman_model->record_count('admin_users');
            $offset                = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
            $all_data              = $this->adminuser_model->search_admin_user_data($key, $config['per_page'], $offset, $this->lang->default_lang_id);
        }
        $this->pagination->initialize($config);

        $all_language_data  = get_admin_lang_data(array('admin_static_links', 'admin_admin_users'), $this->lang->default_lang_id);
        ...
        $pageData = array(
            ...
            'title'                 => get_page_title('admin_users', 'admin_title'),
            'lang_id'               => $this->lang->default_lang,
            'lang_num'              => $this->lang->default_lang_id,
            'active'                => 'adminuser',
            'addscripts'            => 'adminuser',
            ...
            'admin_admin_users'     => $all_language_data['admin_admin_users']
        );

        $this->load->view('admin/common/header', $pageData);
        $this->load->view('admin/common/left_menu', $pageData);
        $this->load->view('admin/adminuser/adminuser_list', $pageData);
        $this->load->view('admin/common/footer', $pageData);
    }

    /**
     * Method delete_adminuser
     *  This Function delete single  row as per the  id passed in the  parameter.
     * @param $id $id [This Parameter is the row id of the admin_users table. ]
     *
     * @return void
     */
    function delete_selected_adminuser() {
        $access = validatePageAccess('adminuser');
        //  this function validate the access of this page for current logged admin user.
        ...
        if ($this->input->post('DeleteAll')) {
            ...
            $this->comman_model->deleteAllDataWithLang('admin_users');
        }

        if ($this->input->post('DeleteSelected')) {
            // this function delete all users as per the users ids posted  in the post parameter.
            $adminUserIds = $this->security->xss_clean($this->input->post('deleteitem'));
            $this->comman_model->deleteAllById('admin_users', $adminUserIds, 'id');
        }
        ...
        redirect(base_url(). 'admin/' . $this->lang->default_lang . '/adminuser');
    }
    ...
    function delete_adminuser($id) {
        ...
        $this->comman_model->delete_where('admin_users', array('id' => $id));
    ...
    function add_adminuser() {
        ...
        if ($this->input->post('operation')) {
            ...
            // this code  save user in the admin_users table 
            $result = $this->comman_model->add('admin_users', $post_data);
        ...
        $all_language_data  = get_admin_lang_data(array('admin_static_links', 'admin_admin_users', 'cart_instruction'), $this->lang->default_lang_id);
        $plang = $this->comman_model->getPrimaryLang();
        ...
        $pageData = array(
            ...
            'lang_num'              => $this->lang->default_lang_id,
            'active'                => 'adminuser',
            'sub_menu'              => 'add_adminuser',
            'addscripts'            => 'add_adminuser',
            'primary_lang'          => !empty($plang) ? $plang['short_code'] : 'en', 
            'country_data'          => $this->comman_model->get_row_array('country', '*', array('status' => 1)),
            'admin_static_links'    => $all_language_data['admin_static_links'],
            'admin_admin_users'     => $all_language_data['admin_admin_users'],
            ...
            'roles'                 => $this->adminrole_model->getRoles(),
            'admin_roles'           => $this->adminrole_model->getalladminroles($this->lang->default_lang_id),
        ...
        $this->load->view('admin/common/header', $pageData);
        $this->load->view('admin/common/left_menu', $pageData);
        $this->load->view('admin/adminuser/adminuser_form', $pageData);
        $this->load->view('admin/common/footer', $pageData);
    }
     ...
     * @return void
     */
    function edit_adminuser($id = false) {
        if (!$id) {
        ...
        if ($this->input->post('operation')) {
            ...
            $this->comman_model->update_data_by_id('admin_users', $post_data, 'id', $id);

            $sessiondata = $this->session->all_userdata();
            // this code update the data in the session if user is already logged in
            if ($id == $sessiondata['id']) {
                ...
                $session_data = array(
                    ...
                    'telephone'     => $post_data['telephone'],
                    'email'         => $post_data['email'],
                    'page_access'   => $sessiondata['page_access'],
                    'admin_validuser_data' => $admin_session_data
                );
        ...
        $pageData = array(
            ...
            'roles'                 => $this->adminrole_model->getRoles(),
            'admin_roles'           => $this->adminrole_model->getalladminroles($this->lang->default_lang_id),
            'edit_data'             => $this->comman_model->get_data_by_id('admin_users', array('id' => $id))
        );
    ...
    public function checkEmailExists($id = '') {
        ...
        if($email && $id){
            $result = $this->comman_model->get_data_by_id('admin_users', array('id' => $id));
            if ($email == $result['email']) {
                // if not  exist than this code return true
                echo json_encode(TRUE);
            }else{
                // this function check is email  exist in the table  or not.
                $exists = $this->comman_model->check_row_exists('admin_users', array('email' => $email));
                if ($exists) {
    ...
    public function checkPhoneExists($id = '') {
        ...
        if($id && $country_code && $telephone){
            ...
            if ($country_code == $result['country_code'] && $telephone == $result['telephone']) {
                ...
                $exists = $this->comman_model->check_row_exists('admin_users', array('country_code' => $country_code, 'telephone' => $telephone));
                if ($exists) {
                    // if exist than this code return false
                    echo json_encode(FALSE);
                } else {
