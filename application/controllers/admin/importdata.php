...
class Importdata extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        //AR set mysql timezone
        if (ENVIRONMENT === 'development' && $_SERVER['HTTP_HOST'] == 'localhost:8080') {
    ...
    public function textcsv(){

        $importfilePath = 'C:\xampp\htdocs\shopameleco\assets\uploads\importproduct\productdata\2024-11-26\1732653565.csv';
        $importfilePath = 'C:\xampp\htdocs\shopameleco\assets\uploads\importproduct\productdata\2024-11-26\1732654049.csv';
        $importfilePath = 'C:\xampp\htdocs\shopameleco\assets\uploads\importproduct\productdata\2024-09-09\test.csv';
         ...
         $rowheader = str_getcsv($raw_string_header);
          echo "<pre>";
          //echo "raw_string_header<br/>";
          //print_r($raw_string_header);
          //echo "<br/>rowheader<br/>";
          print_r($rowheader);
          echo "</pre>";
          echo "<pre>";
        ...
        while (($raw_string = fgets($handle)) !== false) {
            ...
            echo "<br> row $lineNumber :<br/>";
            if(count($rowheader) != count($row)){ //AR condition
                print_r($row);
            }//AR condition
            ...
            echo "<br> Single row $lineNumber :<br/>";
            //print_r($singlerow);
            $lineNumber++;
            $rowNumber++;
        }
    ...
    public function import_products()
    {
        ...
        if ($this->input->post('operation')) {
            ...
            if (!empty($_FILES['images_zip']['name'])) {
                ...
                if ($_FILES['images_zip']['size'] > 400000000) { // 400MB limit
            ...
            if (!empty($_FILES['importproduct_file']['name'])) {
                ...
                if ($_FILES['importproduct_file']['size'] > 400000000) { // 400MB limit
    ...
    public function import_csv_data($importfilePath, $product_folder_images, $request_id, $log_file)
    {
        ...
        while (($raw_string = fgets($handle)) !== false) {
            ...
            // echo $singlerow[$admin_products['template']['front']];
            ...
            if ($singlerow[$admin_products['category_name']['front']]) {
                ...
                if (array_key_exists($vehicle_category_col, $categories)) {
                    ...
                    $vehicle_category_col = (strpos($vehicle_category_col, ',') !== false) ? '' : $vehicle_category_col; //AR
                    ...
                    if ($category_row) {
                        ...
                        $category_id = ($category_id)?$category_id:0;// by AR
                    }
                ...
                // If category_name is not provided, set category_id to 0. 
                $category_id = 0;
            }
            ...
            if ($singlerow[$admin_products['product_model_name']['front']]) {
                ...
                //echo "<h3>Model_name:  ".$singlerow[$admin_products['product_model_name']['front']]."</h3>";//AR
                //echo "<br/>Model_name language".$singlerow[$model_lang_column]."<br>";
            ...
            echo "<h3>Model ID : ".$model_id." , Name:  ". $model_name_col."</h3>";//AR
            ...
            echo "<h3>Product Type ID : ".$product_type_id. ", Name: ".$product_type_col."</h3>";//AR
            ...
            echo "<h3>Product nature ID : ".$nature_id. ", Name: ".$nature_type_col."</h3>";//AR
            ...
            if ($singlerow[$admin_products['kgt_ref_no']['front']]) {
                ...
                if ($product_row) {
                    ...
                    $update_array = array(
                        ...
                        'seo_meta_desc' => $singlerow['SEO_Description'],
                        'seo_meta_keywords' => $singlerow['SEO_Meta_Keywords'],
                    );
                    ...
                    if ($row_validated == "1") {
                        ...
                        $replenishment_order_date= date('Y-m-d', strtotime($replenishment_order_date));
                        ...
                        $update_array = array('availability' => $cart_instruction['backorder_accept_msg']['front'],
                            ...
                            'replenishment_order_date' => $replenishment_order_date,
                    ...
                    $product_data = array(
                        ...
                        'packageId' => $singlerow[$admin_products['package_multiple']['front']],
                        'display_kondarsoft' => $singlerow[$admin_products['display_kondarsoft']['front']],
                        'template' => $singlerow[$admin_products['template']['front']],//AR
                        "pushed_status" => "0",
                        'seo_meta_desc' => $singlerow['SEO_Description'], //AR
                        'seo_meta_keywords' => $singlerow['SEO_Meta_Keywords'],//AR
                    );
                ...
                echo "<h2>Product ID: ". $product_id .", Name: ". $singlerow[$admin_products['part_name']['front']] .",<br/><br/>KGS Ref Number: " .$produt_number_col."</h2><hr/>"; //AR
                ...
                if ($product_id) {
                    ...
                    #bof Parent
                    //$singlerow[$admin_products['parent_multiselect']['front']]
                    if($singlerow['Parent_Products']) {

                            $parent_all = explode("#", $singlerow['Parent_Products']);
                            // this code executed when user chosse the parent product 
                            $product_parent =  $singlerow['Parent_Products'];
                            if (isset($product_parent) && $product_parent != '') {
                                $product_parent = explode('#', $product_parent);
                                //print_r($product_parent);
                                if (!empty($product_parent)) {
                                    $result = $this->comman_model->delete_where('tbl_product_parent', array('product_id' => $product_id));
                                    foreach ($product_parent as $pparent) {
                                        $pparent_data = array();
                                        $parent_product_id= $this->product_model->ar_getProductIdByRefNum(trim($pparent));
                                        if($parent_product_id){
                                            $pparent_data = array(
                                                'product_id'        => $product_id,
                                                'parent_product_id' => $parent_product_id
                                            );
                                           // print_r($pparent_data);
...
//         exit;
        ...
        $header[] = 'Parent_Products';//AR
        $header[] = 'SEO_Description';//AR
        $header[] = 'SEO_Meta_Keywords';//AR
        $rowheader = $header;
        ...
        foreach ($product_table_data as $value) {
            ...
            $ar_all_parents = $this->product_model->getAllEditProducts($value['id']); //AR
            $ar_product_info = $this->product_model->ar_getProductInfoById($value['id']); //AR
            unset($value['id']);
            ...
            foreach ($all_attributes as $single_att) {
                ...
                if ($items_name[$single_att['item_id']]['item_type'] == "product_group") {
                    ...
                    if ($items_name[$single_att['item_id']]['field_type'] == "dropdown") {
                       // $value[$items_field[$single_att['item_id']]][] = $single_att['value'];
                        $value[$items_field[$single_att['item_id']]][] =  preg_replace('/\r?\n/', '', $single_att['value']); //AR
                    } else {
                        //$value[$items_field[$single_att['item_id']]] = $single_att['value'];
                        $value[$items_field[$single_att['item_id']]] = preg_replace('/\r?\n/', '', $single_att['value']); //AR
                    }
            ...
            foreach ($attributes['attributes'] as $attr_key_name) {
             ...
             //AR Parent
            if (count($ar_all_parents) > 0) {
                $ar_all_parents_ref=array();
                foreach ($ar_all_parents as $pe) {
                    $ar_all_parents_ref[] = $this->product_model->ar_getProductRefNumById($pe['parent_product_id']);
                }
               //$single_row[$admin_products['parent_multiselect']['front']] = '';
                $single_row['Parent_Products'] = @implode("#",$ar_all_parents_ref);
            }
          
            $single_row['SEO_Description'] =$ar_product_info['seo_meta_desc']; //AR
            $single_row['SEO_Meta_Keywords'] =$ar_product_info['seo_meta_keywords']; //AR
            fputcsv($handle, $single_row);
        }
    ...
    public function run_import_cron()
    {
        ...
        if ($request_data && empty($current_process)) {
            ...
            if (!file_exists($product_csv_path) && !file_exists($product_csv_path)) {
             ...
             //   echo "<br/>run_import_cron ------ product_csv_path not exist<br/>";
                // if file csv file not exist than this function return error
                ...
                $this->comman_model->update_column("import_requests", array("id" => $request_data['id']), array("status" => "2", "new_status" => "a"));
                exit;
            } else {
                if (!empty($product_zip_name)) {
                  //  echo "<br/>run_import_cron ------ product_zip_name  exist</br/>";
                    // Extract zip to for images and check folder exist in the zip folder
                    $zip = new ZipArchive;
                    $res = $zip->open($product_zip_path);
                    if ($res === true) {
                        ...
                        $this->customlog->write_log(date('y-m-d h:i:s') . " Exit from the function", "newlog");
                        //$this->comman_model->update_column("import_requests", array("id" => $request_data['id']), array("status" => "2"));
                        $this->comman_model->update_column("import_requests", array("id" => $request_data['id']), array("status" => "2", "new_status" => "b"));
                        exit;
                    }
                    ...
                    if (!file_exists($product_folder_images)) {
                        ...
                        $this->customlog->write_log(date('y-m-d h:i:s') . " Exit from the function", "newlog");
                        //$this->comman_model->update_column("import_requests", array("id" => $request_data['id']), array("status" => "2"));
                        $this->comman_model->update_column("import_requests", array("id" => $request_data['id']), array("status" => "2", "new_status" => "c"));
                        exit;
                    }
                } else {
                ...
                // Extract zip process is done
              //  echo "<br/>run_import_cron ------ executing csv</br/>";
                ...
                if ($csv_update['status'] == 1) {
                 //   echo "<br/>run_import_cron ------ csv_update  success</br/>";
                    ...
                    //AR SET flash message
                    $this->session->set_flashdata('success', "Total rows iterated =>" . $csv_update['rowsiterated'] . " and " . "rows created =>" . $csv_update['recordscreated'] . " scuccefully");
                    ...
                    $this->comman_model->update_column("import_requests", array("id" => $request_data['id']), array("status" => "2", "rowsiterated" => $csv_update['rowsiterated'], "productcreated" => $csv_update['recordscreated'],"new_status" => "d")); //AR
                    echo '<br/><hr/><a href="'.base_url() . 'admin/' . $lang_id . '/importdata"><button  class="btn btn info">Back to Import Products request list</button></a><br/>';//AR will set redirect after the test 
                   exit;
                } else {
                  //  echo "<br/>run_import_cron ------ csv_update  failed</br/>";
                    // this function return error message in json format when csv data is not uploaded successfully
                    $this->customlog->write_log($product_csv_path . "file  data not added added", "newlog");
                    $this->customlog->write_log(date('y-m-d h:i:s') . " Exit from the function", "newlog");
                    //$this->comman_model->update_column("import_requests", array("id" => $request_data['id']), array("status" => "2"));
                    $this->comman_model->update_column("import_requests", array("id" => $request_data['id']), array("status" => "2","new_status" => "f")); //AR
                    echo '<br/><hr/><a href="'.base_url() . 'admin/' . $lang_id . '/importdata"><button  class="btn btn info">Back to Import Products request list</button></a><br/>';//AR will set redirect after the test 
                    exit;
                }
            }
            ...
            echo "No data waiting for import";
            echo '<br/><hr/><a href="'.base_url() . 'admin/' . $lang_id . '/importdata"><button  class="btn btn info">&laquo; Back to Import Products request list</button></a><br/>';//AR will set redirect after the test 
                        ...
                        foreach ($all_product_models as $singlemodel) {
                            ...
                            if ($singlemodel['category_id']) {
                                if (array_key_exists($singlemodel['category_id'], $categories)) {
                                    ...
                                    $cat_data['VehicleType_Photo'] = $singlemodel['VehicleType_Photo'];///AR
...
//			echo '<pre>';print_r($product_model_items);print_r($models);echo '</pre>';
    ...
    public function auto_packing_boxes()
    {
        ...
        $log_file = '';
       // echo 'empty_products: '. count($empty_products); //AR
       // exit;
