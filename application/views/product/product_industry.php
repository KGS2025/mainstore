...
<div class="mainContent px-3 px-lg-5">
    ...
    <?php if ($hide_category == "0") { ?>
        <form action="<?php echo base_url() . $lang_id . '/'; ?>products/vehicle_type" id="products_type_list_form" enctype="multipart/form-data" method="post">
        <?php  } else { ?>
            ...
            <?php } ?>
            <input type="hidden" name="filter_option" value="industry-type" />
                ...
                <div class="car-lists text-center productbaselisting mb-4">
                    ...
                    <?php if ($this->config->item('show_dropdown_description') == "1") { ?> 
                            ...
                            <div class="control-group" style="width: 100%"> 
								...
								<div class="controls">
									<select class="group_new_drop_ind focustip span12" name="industry_type[]" multiple="multiple" required>
									   </select>
								</div>
                            ...
                            <?php foreach ($industry_list as $inds_type) { ?>
                                ...
                                <div class="ProductBlock">
                                    <div class="VehicleBlockItems h-float">
                                        <div class="pro-item vehicle_category_image_wrap stepind ">
                                            <div class="overflowhidden">
                                                <a href="javascript:void(0);" class="product_image_wrap" data-rel="<?php echo $inds_type['id']; ?>">
                                                    <?php if (isset($inds_type['icon']) && $inds_type['icon'] != '' && file_exists("assets/uploads/industries/" . $inds_type['icon'])) { ?>
                                                        <img src="<?= $this->session->userdata('default_image'); ?>" data-img="<?php echo asset_url(); ?>assets/uploads/industries/<?php echo $inds_type['icon']; ?>" class="img-responsive" alt="image-<?php echo $inds_type['id']; ?>" id="image_id_<?php echo $inds_type['id']; ?>" />
                                                    <?php } else { ?>
                                                ...
                                                <input type="hidden" name="industry_type[]" value="<?php
                                                                                                    echo $inds_type['id'];
                                                                                                    ?>" class="product_types_id">
                                            </div>
                                            <div class="clearfix"></div>
                                            <a href="javascript:void(0);" class="btn  actn-btn"><?php echo $inds_type['lang_name'] ? $inds_type['lang_name'] : $inds_type['name']; ?> <span class="element_desc"> <?php echo $inds_type['description']; ?> </span> </a>
