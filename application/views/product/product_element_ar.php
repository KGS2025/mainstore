...
<table class="table table-bordered my-table model_table_ product_display_final">
    ...
    echo $general_instruction->requestprice;
                    ...
                    <span class="childparent-arrow" style="width:200px;color:#000;">
                        ...
                        -->
                        <span id="ar-child-hide-<?php echo $product->id; ?>" class="ar-child-hide<?php if($ar_display_type=='child'){echo ' opened';}?>" element-count="<?=$i?>" element-total-parent="<?=count($parent_products)?>" style="padding:20px;cursor:pointer;"> ↑ </span>
                        </td><?php } ?>
                        ...
                        -->
                            <span id="ar-child-show-<?php echo $product->id; ?>" class="ar-child-show<?php if($ar_display_type=='parent'){echo ' opened';}?>" element-count="<?=$i?>" element-total-child="<?=count($child_products)?>" style="padding:20px;cursor:pointer;"> ↓ </span>
                        </td><?php } ?>
    ...
    ?>
        ...
        ?>
            ...
            <td class="font575757 width60px">
                <span id="ar_tbody_<?php echo $product->id; ?>" class="item_count_product ar_tbody" style="display:none;"><?php //echo $i; ?></span>
               ...
               $ar_tbody_display='';
                ...
                $ar_parents_childs['childs']=array();
                    ...
                    if($ar_parents_childs['parents'] || $ar_parents_childs['childs']){
                        ...
                        if($ar_count == 1){
                            echo '<br/>The parent of '.$product->kgt_ref_number.' is: '.implode(", ",$ar_parents_childs['parents']); 
                        }else{
                            echo '<br/><br/>The parents of '.$product->kgt_ref_number.' are: '.implode(", ",$ar_parents_childs['parents']); 
                        }
                        } 
                        if($ar_parents_childs['childs']){
                            $ar_count=count($ar_parents_childs['childs']);
                            if($ar_count == 1){
                            echo '<br/>The child of '.$product->kgt_ref_number.' is: '.implode(", ",$ar_parents_childs['childs']); 
                            }else{
                            echo '<br/><br/>The childs of '.$product->kgt_ref_number.' are: '.implode(", ",$ar_parents_childs['childs']); 
                            }
                        } 
                    }
                ###########eof new display ####
