                ...
                <div class="body">
                    <div class="container">
						...
						<?php }}else{
                                        ...
                                        <div id="ar_product_parent_error" style="color:red;padding:5px;"></div>

                                        <input type="hidden" id="product_parent" name="product_parent" value="<?php if (!empty($values)) {echo implode(',', $values); } ?>" />
...
/////////////////////////////////////
 ...
 /////////////////////////bof auto complete ////////////////////////
 // Initialize SelectPure
var autocomplete = new SelectPure(".autocomplete-select", {
    options: <?php echo $options; ?>, // JSON encoded options array
    value: <?php echo $values; ?>, // JSON encoded initial selected values
    multiple: true, // Enable multiple selections
    autocomplete: true, // Enable autocomplete feature
    icon: "fa fa-times", // Icon for removing selections
    onChange: value => {
        // Triggered when the selected value changes
        $("#ar_product_parent_error").html('');
      //  alert(value); // Show the updated value in an alert
        $("#product_parent").val(value.join(',')); // Update hidden input with selected values as a comma-separated string

        // AJAX call to handle backend logic
        const ar_url = base_url + lang_id + '/part_relation/ar_checkProductsParent/<?php echo $edit_data['id']; ?>';
        $.ajax({
            ...
            success: function (response) {
                ...
                // Display complex relation status
              //  alert('complexRelation: ' + response.complexRelation);

                // Handle updated and removed values
                const newValue = response.updatedValues || [];
                const removedValues = response.removedValues || [];

                // Show removed values (if any) as a message
                if (Object.keys(removedValues).length > 0) {
                    $("#ar_product_parent_error").html(
                        "Has a complex relaion: " + JSON.stringify(removedValues)
                    );
                }

                if (Object.keys(removedValues).length > 0) {
                    const removedSKUs = Object.values(removedValues).join(', '); // Extract and join SKU values
                    $("#ar_product_parent_error").html(
                        removedSKUs  + " have been removed for it's complex relation: "
                    );
                }

                // Update the hidden input field
                $("#product_parent").val(newValue.join(','));

                // Update the SelectPure widget with the new values
                autocomplete._updateSelectedValues(newValue); // Custom function for updating SelectPure
            },
            error: function (xhr, status, error) {
 ...
 // Custom method to update SelectPure selected values
    autocomplete._updateSelectedValues = function (newValues) {
        // Clear current selected values
        const selectedContainer = document.querySelector(".select-pure__select--multiple .select-pure__label");
        selectedContainer.innerHTML = ""; // Clear all current selections

        // Update with new selected values
        newValues.forEach(value => {
            const option = <?php echo $options; ?>.find(opt => opt.value == value); // Find matching option
            if (option) {
                const selectedLabel = document.createElement("span");
                selectedLabel.classList.add("select-pure__selected-label");
                selectedLabel.innerHTML = `${option.label}<i class="fa fa-times" data-value="${value}"></i>`;
                selectedContainer.appendChild(selectedLabel);
            }
        });

        // Update the internal state (if necessary)
        autocomplete.value = newValues;
    };
