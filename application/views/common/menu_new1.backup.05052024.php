					...
					<div id="nav-global-location-slot">
							<span id="nav-global-location-data-modal-action" class="a-declarative nav-progressive-attribute" data-a-modal="" data-action="a-modal">
									...
									<div id="glow-ingress-block">
										<div id="glow-ingress-block">
										<div class="nav-line-1 nav-progressive-content" id="glow-ingress-line1">
...
-->
    ...
    echo "active";}?>" id="cart_title">
									...
									<div class="mobile-nav-menu showInMobile">
										<ul class="mobile-lower-menu">
											<?php foreach($all_navigation_data as $nav_bar){?>
												<li>
													<a href="<?php echo $nav_bar->page_url ?>"><?php echo $nav_bar->title ?></a>
												</li>
											<?php }?>
												<li>
											...
											<?php if(count($category_menu)>0){?>												
												<li class="has-mobile-submenu">
													...
													<?php foreach($category_menu as $category){?>
														<li><a onClick="javascript:jumpToCategory('<?php echo $category['id'];?>');"><?php echo strtoupper($category['lang_category_name']?$category['lang_category_name']:$category['category_name']);?></a></li>
													<?php }?>
													</ul>
												</li>
											<?php }?>
											...
											<li class="has-mobile-submenu"><a><?php echo $general_instruction->search_product_by_brand_type;?></a>
												...
												<?php foreach($brand_menu as $brand){?>
													...
													<li><a onClick="javascript:jumpToMaker('<?php echo $brand['id'].'#'.'all';?>');"><?php echo strtoupper($brand['lang_maker_name']?$brand['lang_maker_name']:$brand['maker_name']);?></a></li>																									
												<?php }?>
												</ul>
											</li>
											<?php }?>
			...
			<?php if (getFrontenduserId()) {
					...
					<div class="mobil-cart showInMobile">
						<a href="<?php echo base_url() . $lang_id . '/user/' . 'addpricerequest'; ?>" title="<?php echo $general_instruction->price_request_menu; ?>" class="kgtcart position-relative nav-link  <?php if (isset($pageType) && ($pageType == 'cart')) {
			echo "active";
			}?>" id="cart_title">
							<i class="fa fa-usd" aria-hidden="true"></i>
    ...
    echo "active";
            ...
            <li class="nav-item has-submenu">
				...
				<ul class="submenu">
                    <div>
					<?php foreach($category_menu as $category){?>
                        <li data-image="<?php echo base_url();?>assets/uploads/vehicle_categories/<?php echo $category['vehicle_category_icon']?>"><a class="submenu_a" onClick="javascript:jumpToCategory('<?php echo $category['id'];?>');"><?php echo strtoupper($category['lang_category_name']?$category['lang_category_name']:$category['category_name']);?></a></li>                        
					<?php }?>						
					</div>					
            ...
            <li class="nav-item has-submenu">
                ...
                <ul class="submenu">
                    <div>
						<?php foreach($brand_menu as $brand){?>
							<?php $explode_val = explode(",",$brand['vehicle_category_id']); ?>
							<li data-image="<?php echo base_url();?>assets/uploads/product_maker/<?php echo $brand['maker_logo'];?>"><a class="submenu_a" onClick="javascript:jumpToMaker('<?php echo $brand['id'].'#'.'all';?>');"><?php echo strtoupper($brand['lang_maker_name']?$brand['lang_maker_name']:$brand['maker_name']);?></a></li>                                               
						<?php }?>
					</div>				
                    <div class="submenu-images"><img src="<?php echo base_url();?>assets/uploads/product_maker/<?php echo $brand_menu[0]['maker_logo'];?>"></div>
                </ul>
