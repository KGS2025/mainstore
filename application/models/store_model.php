...
class Store_model extends CI_Model
{
     ...
     * @return void
     */
    function search_store_data($table, $key, $field1, $per_page, $offset, $lang_id)
    {
        $this->db->select('*');
        $this->db->from($table);
        if ($key != "") {
        ...
        foreach ($result as $res) {
            $r = $this->db->query("SELECT c.lang_name FROM store as a, store_country as c WHERE a.id = c.lang_id  AND a.id = '" . $res['id'] . "' AND c.country_id = '" . $lang_id . "' ")->row_array();
